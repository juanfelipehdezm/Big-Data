# -*- coding: utf-8 -*-
"""
Created on Wed Sep 23 11:00:20 2020
...
"""
#con la info impresa en la linea de arriba ,en el situo jsoneditoronline.org sabemos que tipo de data 
structured tiene 

# ya sabemos que es un dic con 2 keys
#print("the type of the firs level is ",type(res))
#print("The keys of the first level are",res.keys())
#como sabemos que queremos el compre y la fecha  en que se creo el tweet , esta se encuentre en "statuses"

res2 = res["statuses"]


print("-------------------------- level 2 -----------------------------")
print("The type of the second level is ",type(res2))
print("the list res2 contain ", len(res2), "tweets")


for tweet in res2[:1]: #sabemos que res2 contiene 3 items que son 3 tweets y su info
    print("----- ----------level 3 : a tweet info---------------------- ")
    print(type(tweet)) #para facilitar lo hacemos con el primer tweet y ese nos dice que es un dic
    print(tweet.keys())
    print("----------------level 4 : users info---------------------------")
    user = tweet["user"]
    print(type(user)) #como es un dic pedimos las keys
    print(user.keys())
    print("----------------inside level 4 --------------------------------")
    print(user["screen_name"],"   ",user["created_at"])
    
    
UNA VEZ ENCONTRAMOS LO QUE BUSCAMOS , COMPILAMOS EL CODIGO A ALGO MAS SENCILLO ASI :::
    
"""
# TODO LO ANTERIOR FUE PARA DEPURAR

res2 = res["statuses"] 


for tweet in res2:
    print(tweet["user"]["screen_name"],"   ",tweet["user"]["created_at"])